...
export const getEvents = (data: INewDeskFormValues["schedule"]) => {
  ...
  for (const day of period) {
    ...
    for (const event of oneDayEvents) {
      const start = setDayOfYear(
        setYear(event.start, getYear(day)),
        getDayOfYear(day)
      );
      const end = setDayOfYear(
        setYear(event.end, getYear(day)),
        getDayOfYear(day)
      );
      output.push({
...
const getPureSortedBreaks = (
  data: INewDeskFormValues["schedule"]["breaks"]
) => {
  const breaks = [
    ...(
